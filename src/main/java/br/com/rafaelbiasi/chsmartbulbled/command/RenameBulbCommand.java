...
public class RenameBulbCommand implements BulbCommand {
	public static final byte COMMAND_SIZE = 0x4C; // 76 bytes
	...
	public byte[] getCommandBytes() {
		...
		prefixCommand[0x6] = COMMAND_SIZE; //Byte array size (76)
		...
		byte[] endZeroFill = new byte[COMMAND_SIZE - (prefixCommand.length + prefixName.length + nameBytes.length)];
		Arrays.fill(endZeroFill, (byte) 0x00);
