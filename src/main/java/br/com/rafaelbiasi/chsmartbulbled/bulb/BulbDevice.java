...
public class BulbDevice {
	...
	public BulbDevice sceneEffect(SceneEffect sceneEffect, Color color, Speed speed) {
		bulbCommand = new SceneBulbCommand(sceneEffect, color, speed);
		return this;
	}
	...
	public BulbDevice startCustomEffect() {
		...
		Thread.ofPlatform().start(() -> {
			...
			while (customEffectRunning) {
				delay.prepareDelay();
				this.color(customEffect.apply(this, frame++, LocalDateTime.now()));
				this.commit();
